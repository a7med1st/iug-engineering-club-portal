import {
  rateLimitResponse,
  resendPasswordResetRateLimitRules,
...
const GENERIC_MESSAGE =
  "إذا كان البريد مرتبطًا بحساب مؤهل، فسيصلك رمز جديد خلال دقائق.";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const rateLimit = await consumeRateLimits(
      resendPasswordResetRateLimitRules(request, emailResult.email),
    );
    ...
    return privateNoStoreJson(
      { ok: false, error: "تعذر إرسال الرمز الآن. حاول مرة أخرى لاحقًا." },
      { status: 500 },
    );
  }
}
